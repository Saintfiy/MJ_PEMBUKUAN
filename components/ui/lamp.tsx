...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col items-center justify-start overflow-hidden bg-slate-950 w-full z-0 pt-32",
        className
      )}
    >
      {/* ── LAMP LIGHT EFFECT (top portion) ── */}
      <div className="relative flex w-full items-center justify-center isolate z-0 h-72">
        {/* Left cone */}
        <motion.div
        ...
        <div className="absolute inset-auto z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-slate-400 opacity-40 blur-3xl" />
        <motion.div
          initial={{ width: "8rem" }}
          whileInView={{ width: "16rem" }}
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
          className="absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-slate-300 blur-2xl opacity-60"
        />
        <motion.div
          initial={{ width: "15rem" }}
      ...
      <div className="relative z-50 flex flex-col items-center px-5 w-full pb-20">
        {children}
      </div>
    </div>
  );
